...
class Transfer extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      !_.isEqual(this.props.leftTreeData, nextProps.leftTreeData) ||
      !_.isEqual(this.props.rightTreeData, nextProps.rightTreeData)
    ) {
      this.setState({
  ...
  handleTransClick = (key) => {
    ...
    if (key === 'allToRight' || key === 'toRight') {
      ...
      newRightArrayData = compareInitData(
        mergeArrayData(getParentKeyArrayObj(checkedKeys, allArrayData), _.cloneDeep(rightArrayData)),
        _.cloneDeep(rightArrayDataInit),
      );
      ...
      newLeftArrayData = compareInitData(
        mergeArrayData(getParentKeyArrayObj(checkedKeys, allArrayData), _.cloneDeep(leftArrayData)),
        _.cloneDeep(leftArrayDataInit),
      );
  ...
  renderBtns = (btns) => {
    ...
    return btns.map((item) => {
      ...
      let enable =
        ...
        key === 'allToLeft' ||
        key === 'reset' ||
        (key === 'toRight' && leftCheckedKeys.length > 0) ||
        (key === 'toLeft' && rightCheckedKeys.length > 0)
          ? 'enable'
          : 'disable';
      return (
        <Button
          ...
          onClick={
            ...
            (key === 'toRight' && leftCheckedKeys.length > 0) ||
            (key === 'toLeft' && rightCheckedKeys.length > 0)
              ? this.handleTransClick.bind(this, key)
              : () => {
  ...
  render() {
    ...
    return (
      <div ref={this.createRef('treeTransfer')} className="tree-transfer-container" style={{ height: treeHeight }}>
        <div className="tree-transfer-left" style={{ width: treeWidth }}>
          {leftTitle.length > 0 ? (
            <div className="tree-title">
              {leftTitle} <i className="tree-nums">{leftTreeArray.length}</i>
            </div>
          ) : null}
          <Treex
        ...
        <div className="tree-transfer-middle">{this.renderBtns(transferBtns)}</div>
        <div className="tree-transfer-right" style={{ width: treeWidth }}>
          {rightTitle.length > 0 ? (
            <div className="tree-title">
              {rightTitle} <i className="tree-nums">{rightTreeArray.length}</i>
            </div>
          ) : null}
          <Treex
