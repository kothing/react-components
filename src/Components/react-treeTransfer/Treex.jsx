      ...
      expandedKeys = this.props.arrayData
        .map((item) => {
          return item.title.toLowerCase().indexOf(value.toLowerCase()) > -1
            ? getParentKey(item.key, this.props.arrayData)
            : null;
        })
  ...
  render() {
        ...
        if (item.children) {
          return (
            <TreeNode
              ...
              className={item.isNew ? 'tree-li new' : 'tree-li'}
            >
    ...
    return (
      <div className="tree-container" style={this.props.style}>
        ...
        <div
          className="tree-list"
          style={{ height: `${showSearch ? `calc(100% - 46px)` : '100%'}`, marginTop: `${showSearch ? '46px' : '0'}` }}
        >
          <Tree
            checkable={true}
